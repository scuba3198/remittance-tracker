...
export function SettingsMenu() {
	...
	const handleExport = async () => {
		try {
			...
			saveAs(
				blob,
				`remittance-backup-${new Date().toISOString().split("T")[0]}.json`,
			);
		} catch (e) {
						...
						<div className="space-y-4">
							...
							<div className="p-4 bg-secondary/30 rounded-xl border border-border">
								...
								<p className="text-xs text-muted-foreground mb-4 leading-relaxed">
									Download your transaction history as a JSON file to back it up
							...
							<div className="p-4 bg-secondary/30 rounded-xl border border-border">
								...
								<p className="text-xs text-muted-foreground mb-4 leading-relaxed">
									Restore your history from a backup file. Existing transactions
									will be updated.
								</p>
								<input
									type="file"
								...
								{importStatus && (
									<div
										className={`mt-4 text-xs p-3 rounded-lg border flex items-start gap-2 ${
