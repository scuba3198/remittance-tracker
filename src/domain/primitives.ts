...
export const TransactionId = {
    ...
    of: (id: string): Result<TransactionId, DomainError> => {
        ...
        if (!valid.success) {
            return Result.err({ type: 'ValidationError', field: 'id', message: 'Invalid UUID format' });
        }
...
export const TransactionDate = {
    ...
    of: (dateStr: string): Result<TransactionDate, DomainError> => {
        ...
        if (!regex.test(dateStr)) {
            return Result.err({ type: 'ValidationError', field: 'date', message: 'Invalid date format (YYYY-MM-DD required)' });
        }
        ...
        if (isNaN(inputDate.getTime())) {
            return Result.err({ type: 'ValidationError', field: 'date', message: 'Invalid date value' });
        }
...
export const TransactionNote = {
    of: (note: string): Result<TransactionNote, DomainError> => {
        if (note.length > 200) {
            return Result.err({ type: 'ValidationError', field: 'note', message: 'Note must be 200 characters or less' });
        }
        // Basic sanitization could happen here, or validation that it contains no scripts.
        if (/<script/i.test(note)) {
            return Result.err({ type: 'ValidationError', field: 'note', message: 'Invalid characters in note' });
        }
